...
class ProteinGraphDataset(data.Dataset):
    ...
    '''
    # Dataset, Nombre y sequencia en forma de diccionario
    def __init__(self, dataset, index, pdb_file, task_list,
                 num_positional_embeddings=16,
                 top_k=30, num_rbf=16, device="cpu"):
        
        super(ProteinGraphDataset, self).__init__()

        self.dataset = {}                       #No creo que sea necesario usar diccionario, ya veremos
        index = set(index)
        ...
        self.dataset_path = 'Example/structure_data/'   # '../Example/structure_data/'
        self.feature_path = 'Example/prottrans/'

        #self.fasta_file = args.fasta_file          #usar el pbd_sequence
        self.output_prottrans = 'Example/prottrans/'  #lo mismo que feature path
        #self.output_esmfold = args.output_esmfold       ## No vamos a predecir el pbd
        self.output_dssp = 'Example/structure_data/'             #No se q es esta wea pero si lo calcula
        self.task_list = task_list                  #['DNA', 'RNA']

        self.top_k = top_k                      #Parametros ya establecidos
        self.num_rbf = num_rbf                  #Parametros ya establecidos
        self.num_positional_embeddings = num_positional_embeddings      ###Parametros ya establecidos
        self.device = device    
        self.node_counts = [len(self.dataset[ID][0]) for ID in self.IDs]        ###Checar que hace esta wea segun yo numero de aminoacidos
        ...
        self.num_to_letter = {v:k for k, v in self.letter_to_num.items()}           ###Solito se entiende
        
    def __len__(self): return len(self.IDs)         ###Cuantas proteinas va a procesar
    
    def __getitem__(self, idx): return self._featurize_as_graph(idx)      ###Hace la wea de abajo  
    
    def _featurize_as_graph(self, idx):     
        name = self.IDs[idx]                #Nombre de la proteina
        with torch.no_grad():
            if not os.path.exists(self.output_prottrans + name + "pt.npy"):    ##Checa si ya existe el prottrans
                features.get_prottrans(self.dataset, self.output_prottrans)  ##Si no lo calcula
            if not os.path.exists(self.dataset_path + name + "xyz.npy"):
                features.get_pdb_xyz(self.pdb_file,self.dataset, self.dataset_path)  #Darle la secuencia y el xyz
            if not os.path.exists(self.dataset_path + name + "_dssp.npy"):
                features.get_dssp(self.dataset, self.pdb_file, self.output_dssp)  #Darle la secuencia y el xyz
            coords = np.load(self.dataset_path + name + "xyz.npy")
